<header>
    <div class='header-top'>
        <div> <!-- For the current visiting page of the site -->
            <?php
                if($_SERVER['REQUEST_URI'] === '/index.php'){    //home page
                 ...
                 else if($_SERVER['REQUEST_URI'] === '/public/owner/owner_dashboard.php'){
                    echo "Welcome, ".$_SESSION['username'];
                }
            ?>
        </div>
        <div>
        <?php
            if(isset($_SESSION['username']) && isset($_SESSION['userrole'])){ //owner logged in
        ?>     
            <div style="float:right" id="logout">
                <button class ="btn" onclick="window.location.href='/public/owner/owner_dashboard.php'"><img src="/styles/icons/search_icon.svg" class="acc-img"></button>
